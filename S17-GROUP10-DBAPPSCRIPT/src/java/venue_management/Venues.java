...
public class Venues {
    ...
    public int times_rented(int venue_ID, Date date) throws ClassNotFoundException
    {
        int sold = 0;
        ...
        {   
            ...
            PreparedStatement pstmt = conn.prepareStatement("SELECT COUNT(*) AS times_rented FROM Venues v JOIN Concerts c ON v.venue_ID = c.venue_ID WHERE DATE(c.transaction_date) = ? and v.venue_ID = ?");
    ...
    public int merchandise_sold_in_venue(int venue_ID, Date date) throws ClassNotFoundException
    {
        int sold = 0;
        ...
        {   
            ...
            PreparedStatement pstmt = conn.prepareStatement("SELECT SUM(mp.quantity_bought) AS merchandise_sold FROM Venues v JOIN Concerts c ON v.venue_ID = c.concert_ID JOIN Merchandise m ON c.concert_ID = m.concert_ID JOIN Merchandise_Purchases mp ON m.item_ID = mp.item_ID JOIN Transactions tr ON mp.transaction_ID = tr.transaction_ID WHERE v.venue_ID = ? AND DATE(tr.date_of_transaction) = ?");
    ...
    public double cut_earned(int venue_ID, Date date) throws ClassNotFoundException
    {
        double cut = 0;
        ...
        {   
            ...
            PreparedStatement pstmt = conn.prepareStatement("SELECT (SUM(mp.quantity_bought*m.price)*(v.merchandise_cut_percentage*0.01)) AS total_earned FROM Venues v JOIN Concerts c ON v.venue_ID = c.concert_ID JOIN Merchandise m ON c.concert_ID = m.concert_ID JOIN Merchandise_Purchases mp ON m.item_ID = mp.item_ID JOIN Transactions tr ON mp.transaction_ID = tr.transaction_ID WHERE v.venue_ID = ? AND DATE(tr.date_of_transaction) = ?");
            pstmt.setInt(1, venue_ID);
            pstmt.setDate(2, date);
            ResultSet rs = pstmt.executeQuery();
