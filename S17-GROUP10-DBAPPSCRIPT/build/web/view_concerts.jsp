    ...
    <body>
        ...
        <div class="container-wrapper">
                    ...
                    <div class="input-user">
                        <div class="input-details">
                            ...
                            <select id = "concerts" name = "concerts">
                                <option value="all-concerts">(ALL CONCERTS)</option>
                                <option value="upcoming-concerts">(UPCOMING CONCERTS)</option>
                                <option value="cancelled-concerts">(CANCELLED CONCERTS)</option>
                                <option value="completed-concerts">(COMPLETED CONCERTS)</option>
                                <%
                                    C.concert_list();
            ...
            <%
                ...
                if(selected != null)
                {
                    if("all-concerts".equals(selected))
                    {
                        C.concert_list();
                        
                    }
                    else if("upcoming-concerts".equals(selected))
                    {
                        C.upcoming_concert_list();
                    }
                    else if("cancelled-concerts".equals(selected))
                    {
                        C.cancelled_concert_list();
                    }
                    else if("completed-concerts".equals(selected))
                    {
                        C.completed_concert_list();
                    }
                    else
                    {
                        ...
                        C.concert_list();
                            ...
                            <div class="container">
                                ...
                                <%int i = C.get_concert_ID_list().indexOf(selected_concert);%>
                                
                                <br><div class="subtitle"><%=C.get_concert_name_list().get(i)%></div><br><br>
                                <span class="subdetails"><b>Concert ID:</b> <%=C.get_concert_ID_list().get(i)%></span><br>
                                <span class="subdetails"><b>Performer ID:</b> <%=C.get_performer_ID_list().get(i) + " (" + P.find_performer_name(C.get_performer_ID_list().get(i)) + ")"%></span><br>
                                <span class="subdetails"><b>Venue ID:</b> <%=C.get_venue_ID_list().get(i) + " (" + V.find_venue_name(C.get_venue_ID_list().get(i))+ ")"%></span><br>
                                <span class="subdetails"><b>Date Scheduled:</b> <%=C.get_transaction_date_list().get(i)%></span><br>
                                ...
                                <span class="subdetails"><b>Status:</b> <%=C.get_concert_status_list().get(i)%></span><br>
                                <% 
                                if(C.get_concert_status_list().get(i).equals("UPCOMING")) {
                                %>
                                    <form action="cancel_concerts.jsp" method="GET">
                                        <input type="hidden" name="concerts" value="<%= C.get_concert_ID_list().get(i) %>" />
                                        <div class ="button">
                                            <input type = "submit" value = "Cancel">
                                        </div><br>
                                    </form>
                                <%
                                } else System.out.println("NOT UPCOMING");
                                %>
                            </div>
                            <%
                    ...
                    if("all-concerts".equals(selected) || "upcoming-concerts".equals(selected) || "cancelled-concerts".equals(selected) || "completed-concerts".equals(selected))
                    {   for(int i = 0; i < C.get_concert_ID_list().size(); i++)
                        {
                            %>
                            <div class="container">
